...
// Dodawanie produktu
function DodajProdukt($tytul, $opis, $data_wygasniecia, $cena_netto, $podatek_vat, $ilosc, $status_dostepnosci, $kategoria, $gabaryt, $zdjecie) {
    global $db;
    $stmt = $db->prepare("INSERT INTO products (tytul, opis, data_wygasniecia, cena_netto, podatek_vat, ilosc, status_dostepnosci, kategoria, gabaryt, zdjecie) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssddiisss", $tytul, $opis, $data_wygasniecia, $cena_netto, $podatek_vat, $ilosc, $status_dostepnosci, $kategoria, $gabaryt, $zdjecie);
    $stmt->execute();
    $stmt->close();
}
...
// Edytowanie produktu
function EdytujProdukt($id, $tytul, $opis, $data_wygasniecia, $cena_netto, $podatek_vat, $ilosc, $status_dostepnosci, $kategoria, $gabaryt, $zdjecie) {
    global $db;
    $stmt = $db->prepare("UPDATE products SET tytul = ?, opis = ?, data_wygasniecia = ?, cena_netto = ?, podatek_vat = ?, ilosc = ?, status_dostepnosci = ?, kategoria = ?, gabaryt = ?, zdjecie = ? WHERE id = ?");
    $stmt->bind_param("sssddiisssi", $tytul, $opis, $data_wygasniecia, $cena_netto, $podatek_vat, $ilosc, $status_dostepnosci, $kategoria, $gabaryt, $zdjecie, $id);
    $stmt->execute();
    $stmt->close();
}
...
function PokazProdukty() {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<p>Ilość: " . $row['ilosc'] . " | Status: " . $row['status_dostepnosci'] . "</p>";
...
<body onload="startClock()">
    ...
    <h2>Dodaj Produkt</h2>
<form method="POST">
    <label>Tytuł: <input type="text" name="tytul" required></label>
    <label>Opis: <textarea name="opis" required></textarea></label>
    <label>Data wygaśnięcia: <input type="date" name="data_wygasniecia"></label>
    <label>Cena netto: <input type="number" step="0.01" name="cena_netto" required></label>
    <label>VAT: <input type="number" step="0.01" name="podatek_vat" required></label>
    <label>Ilość: <input type="number" name="ilosc" required></label>
    <label>Status: 
    ...
    <label>Kategoria: <input type="number" name="kategoria"></label>
    <label>Gabaryt: <input type="text" name="gabaryt"></label>
    <label>Zdjęcie: <input type="text" name="zdjecie"></label>
